...
public class AddCoinMultiplierCommand {

    public AddCoinMultiplierCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("multipliers").then(Commands.literal("coin").then(Commands.literal("add").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    playerTime.incrementCoinMultiplier(multiplier, player);
                    ...
                    player.displayClientMessage(
                            Component.translatable("chat.notification.sut.coin_multiplier.add", multiplier).withStyle(ChatFormatting.AQUA),
                            false
                    );
                });
            ...
            context.getSource().sendSuccess(
                    () -> Component.translatable("chat.notification.general_command.add_players", "x"+multiplier, "Coin Multiplier", playerNames.toString()).withStyle(ChatFormatting.GREEN),
                    false
            );
