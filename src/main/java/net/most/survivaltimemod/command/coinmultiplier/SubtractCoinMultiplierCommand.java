...
public class SubtractCoinMultiplierCommand {

    public SubtractCoinMultiplierCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("multipliers").then(Commands.literal("coin").then(Commands.literal("subtract").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    playerTime.decrementCoinMultiplier(multiplier, player);
                    ...
                    player.displayClientMessage(
                            Component.translatable("chat.notification.sut.coin_multiplier.subtract", multiplier).withStyle(ChatFormatting.AQUA),
                            false
                    );
                });
            ...
            context.getSource().sendSuccess(
                    () -> Component.translatable("chat.notification.general_command.sub_players", "x"+multiplier, "Coin Multiplier", playerNames.toString()).withStyle(ChatFormatting.GREEN),
                    false
            );
