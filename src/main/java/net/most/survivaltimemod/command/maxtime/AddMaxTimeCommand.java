...
public class AddMaxTimeCommand {
    public AddMaxTimeCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("maxtime").then(Commands.literal("add").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    playerTime.incrementMaxTime(timeToSet, player);
                    ...
                    player.displayClientMessage(
                            Component.translatable("chat.notification.sut.maxtime.add", formattedTime).withStyle(ChatFormatting.AQUA),
                            false
                    );
                });
            ...
            context.getSource().sendSuccess(
                    () -> Component.translatable("chat.notification.general_command.add_players", formattedTime, "Max Time", playerNames.toString()).withStyle(ChatFormatting.GREEN),
                    true
            );
