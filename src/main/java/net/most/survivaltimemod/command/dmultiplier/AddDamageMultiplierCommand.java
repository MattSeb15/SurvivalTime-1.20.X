...
public class AddDamageMultiplierCommand {

    public AddDamageMultiplierCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("multipliers").then(Commands.literal("damage").then(Commands.literal("add").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    playerTime.incrementDamageMultiplier(multiplier, player);
                    ...
                    player.displayClientMessage(
                            Component.translatable("chat.notification.sut.damage_multiplier.add", multiplier).withStyle(ChatFormatting.AQUA),
                            false
                    );
                });
            ...
            context.getSource().sendSuccess(
                    () -> Component.translatable("chat.notification.general_command.add_players", "x"+multiplier, "Damage Multiplier", playerNames.toString()).withStyle(ChatFormatting.GREEN),
                    false
            );
