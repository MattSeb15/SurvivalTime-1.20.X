...
public class InfoTimePlayedCommand {
    public InfoTimePlayedCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("timeplayed").then(Commands.literal("info").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    String currentTimePlayed = FormatTimeType.getFormattedStringByType(FormatTimeType.DEFAULT, (float) playerTime.getTimePlayed());
                    String playerName = player.getGameProfile().getName();
                    int playerIndex = players.stream().toList().indexOf(player) + 1;

                    context.getSource().sendSuccess(() -> Component.translatable("chat.notification.sut.timeplayed.info", playerIndex, playerName,
