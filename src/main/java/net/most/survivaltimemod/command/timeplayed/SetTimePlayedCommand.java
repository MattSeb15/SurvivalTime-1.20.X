...
public class SetTimePlayedCommand {
    public SetTimePlayedCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("timeplayed").then(Commands.literal("set").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    playerTime.setTimePlayed(timeToSet, player);
                    ...
                    player.displayClientMessage(Component.translatable("chat.notification.sut.timeplayed.set", formattedTime).withStyle(ChatFormatting.GREEN), false);
                });

            }
            context.getSource().sendSuccess(
                    ()-> Component.translatable("chat.notification.general_command.set_players", formattedTime,  "Time Played", playerNames.toString()).withStyle(ChatFormatting.GREEN),
                    true
            );
