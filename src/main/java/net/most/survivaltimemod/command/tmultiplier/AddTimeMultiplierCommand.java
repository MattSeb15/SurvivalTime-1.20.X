...
public class AddTimeMultiplierCommand {

    public AddTimeMultiplierCommand(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("sut")
                .requires(
                        (source) -> source.hasPermission(Commands.LEVEL_OWNERS)

                )
                .then(Commands.literal("multipliers").then(Commands.literal("time").then(Commands.literal("add").then(
        ...
        try {
            ...
            for (ServerPlayer player : players) {
                player.getCapability(PlayerTimeProvider.PLAYER_TIME_CAPABILITY).ifPresent(playerTime -> {
                    playerTime.incrementTimeMultiplier(multiplier, player);
                    ...
                    player.displayClientMessage(
                            Component.translatable("chat.notification.sut.time_multiplier.add", multiplier).withStyle(ChatFormatting.AQUA),
                            false
                    );
                });
            ...
            context.getSource().sendSuccess(
                    () -> Component.translatable("chat.notification.general_command.add_players", "x"+multiplier, "Time Multiplier", playerNames.toString()).withStyle(ChatFormatting.GREEN),
                    false
            );
